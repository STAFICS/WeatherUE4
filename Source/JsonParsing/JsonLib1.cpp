...
{
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, "Database Open");
	Database.Open(*GetPathDB(), nullptr, nullptr);
...
FString UJsonLib1::GetPathDB()
{
	...
	return FPaths::ProjectDir() / "Content" / "db.db";
...
{
	FString QueryInsert = FString::Printf(TEXT("INSERT INTO Cities (id, NameCity, temp, timestamp) VALUES (%i, \"%s\", %i, %i);"), IdCity, *NameCity, temp, timestamp, IdCity);
...
void UJsonLib1::InsertIntoWeatherTable(FWeatherMain WeatherData)
{
	...
	//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, Query);
